...
Olen oppinut paljon JS-kirjastojen käyttämisestä sekä lisää JSONin parsimisesta. Jpuduin turvautumaan hivenen tekoälyyn ongelmien ratkaisemisessa, mutta perusperiaatteen ymmärsin itse. Olisin varmasti oppinut enemmän, jos olisin pystynyt ajankäytöllisesti keskittymään paremmin koodaamiseen tänä keväänä. Se ei kuitenkaan ollut mahdollista. Paljon olen kaikesta huolimatta oppinut.
...
[Uimavedet 2.0] on sovellus, joka kertoo käyttäjälle reaaliaikaista dataa pääkaupunkiseudun uimarantojen vesien lämpötiloista ja diagrammin edellisen kahden vuoden mitatuista lämpötiloista uimarantakohtaisesti. Tiedot päivittyvät noin 30 minuutin välein.
...
## Teknologiat
Käytin seuraavia teknologioita: `html`, `css` ja `Java Script` ja lisäksi `Bootstrap` sekä `Chart.js` -kirjastoja.
